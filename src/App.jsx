...
function parseShafts(rows, fileName){
  ...
  if(isCasarao){
    ...
    for(let i=hiIdx+1;i<rows.length;i++){
      const r=rows[i];
      if(!r[0]||/^[,\s]*$/.test(r[0])||/total|legenda/i.test(r[0])) continue;
...
function parseCapiacos(rows, fileName){
  ...
  const headerIdx=rows.findIndex(r=>{
    ...
    const torrI=r.findIndex(c=>/^torre$/i.test(c));
    return aptoI!==-1&&torrI!==-1;
  });
  ...
  const headers=hRow.slice(ambStart).map(fix).filter(Boolean);
  for(let i=headerIdx+1;i<rows.length;i++){
    const r=rows[i];
    const apto=san(r[aptoCol]), torre=san(r[torreCol]);
    if(!apto||!isApto(apto)) continue;
    if(!torre) continue;
...
function parseEsquadrias(rows, fileName){
    ...
    while(ci<hRow.length){
      ...
      if(isAptoCell||isEmptyBTorre){
        ...
        for(let x=ambStart+1;x<hRow.length;x++){
          ...
          if(/^apto$/i.test(cx)||(cx===""&&/^torre$/i.test(cx1))){ ambEnd=x; break; }
        }
        const headers=hRow.slice(ambStart,ambEnd).map(h=>san(fix(h)));
        blocos.push({ aptoCol, torreCol, ambStart, ambEnd, headers });
        ci=ambEnd;
      } else { ci++; }
    }
    return blocos;
  ...
  const headerLines=rows.reduce((acc,r,i)=>{
    const hasTorre=r.some(c=>/^torre$/i.test(san(c)));
    const hasAmb  =r.filter(c=>san(c).length>2&&!/^apto$|^torre$/i.test(san(c))).length>=3;
    if(hasTorre&&hasAmb) acc.push(i);
    return acc;
  },[]);

  for(const hi of headerLines){
    const blocos=detectBlocos(rows[hi]);
    if(!blocos.length) continue;
    for(let i=hi+1;i<rows.length;i++){
      const r=rows[i];
      for(const b of blocos){
        const apto=san(r[b.aptoCol]), torre=san(r[b.torreCol]);
        if(!apto||!isApto(apto)) continue;
        if(!torre||!isTorre(torre)) continue;
        b.headers.forEach((amb,j)=>{
          const val=san(r[b.ambStart+j]);
          if(!val||!STATUS_VALIDOS.has(val)) return;
          result.push({ tipo:"esquadrias", torre:torre.toUpperCase(), apto, pav:pav(apto), ambiente:fix(amb), status:val, fonte:fileName });
        });
      }
    }
  }
...
const CLASS = {
  shaft:{
    ...
    colors:{ A:"#16a34a", FS:"#f59e0b", FC:"#2563eb", "N/A":"#94a3b8", "N/V":"#64748b" },
  },
  capiacos:{
    ok:["OK"], warn:["Q","N","Q.I","F"], na:["N/V","N/A"],
    labels:{ OK:"Correto", Q:"Sem queda", N:"Desnivelado", "Q.I":"Queda invertida", F:"Falta fachada", "N/V":"N/V" },
    colors:{ OK:"#16a34a", Q:"#dc2626", N:"#f59e0b", "Q.I":"#b91c1c", F:"#ea580c", "N/V":"#94a3b8" },
  },
  passantes:{
      ...
      {entries.map(([k,v],idx)=>{
        ...
        return(
          <div key={k} style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:140,fontSize:12,color:C.white,textAlign:"right",flexShrink:0}}>{labels[k]}</div>
...
export default function App(){
  ...
  async function runAI(){
    ...
    try{
      ...
      if(data.error){
        setAiText(`Erro da API: ${data.error}\n${data.raw||""}`);
      } else if(!data.content){
        setAiText(`Resposta inesperada:\n${JSON.stringify(data,null,2)}`);
      } else {
        const text=data.content.filter(c=>c.type==="text").map(c=>c.text).join("\n");
        setAiText(text||`Content vazio:\n${JSON.stringify(data.content,null,2)}`);
      }
      ...
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        ...
        <div style={{background:C.card,borderRadius:18,padding:20,marginBottom:20,border:`1px solid ${C.border}`}}>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"center"}}>
            ...
            {allRows.length>0 && <Btn onClick={()=>exportCSV(allRows)} color={C.purple}>⬇ Exportar CSV</Btn>}
            {allRows.length>0 && (
        ...
        {allRows.length>0 && <>
          ...
          {tab==="shafts" && <>
            ...
            <Box title="Detalhe por Apartamento"><TabelaApto data={shaftData}/></Box>
          </>}
          {tab==="capiacos" && <>
            ...
            <Box title="Detalhe por Apartamento"><TabelaApto data={capData}/></Box>
          </>}
          {tab==="passantes" && <>
            ...
            <Box title="Detalhe por Apartamento"><TabelaApto data={passData}/></Box>
          </>}
          {tab==="esquadrias" && <>
            ...
            <Box title="Detalhe por Apartamento"><TabelaApto data={esqData}/></Box>
          </>}
          {tab==="ia" && (
